...
class Topics {
    ...
    public function getTopic($id) {
        $query = Database::getFactory()->getConnection(SETTING["db-name"])->prepare("SELECT forum_topics.id, forum_topics.title, forum_topics.content, accounts.steamName, accounts.steamID, accounts.steampfpmed, forum_topics.posted FROM forum_topics INNER JOIN accounts WHERE forum_topics.id = :id AND accounts.steamID = forum_topics.author LIMIT 1");
    ...
    public function getLastTopic($section) {
        $query = Database::getFactory()->getConnection(SETTING["db-name"])->prepare("SELECT forum_topics.id, forum_topics.title, forum_sections.id as `sID`, forum_sections.title as `sTitle`, accounts.steamName, accounts.steamID, accounts.steampfpmed, forum_topics.posted FROM forum_topics INNER JOIN accounts INNER JOIN forum_sections WHERE forum_sections.id = forum_topics.section_id AND accounts.steamID = forum_topics.author AND section_id = :id ORDER BY posted DESC LIMIT 1");
        $query->execute(array(":id" => $section));
